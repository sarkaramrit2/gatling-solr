...
if [ "$IMPLICIT_CLUSTER" = true ] ; then
    ...
    docker exec kubectl-support kubectl exec -n ${GCP_K8_CLUSTER_NAMESPACE} ${EXT_SOLR_NODE_POD_NAME} -- /opt/solr/bin/solr create -c wiki -s $((NUM_SHARDS)) -rf $((NUM_REPLICAS)) -d /opt/solr/collection-config/
fi
...
if [ ! -z "${REMOTE_INDEX_FILE_PATH}" ]; then
  ...
  for (( c=0; c<${GATLING_NODES}; c++ ))
  do
     docker exec -d kubectl-support kubectl exec -n ${GCP_K8_CLUSTER_NAMESPACE} gatling-solr-${c} -- mkdir -p /opt/gatling/user-files/data/external/
     docker exec -d kubectl-support kubectl exec -n ${GCP_K8_CLUSTER_NAMESPACE} gatling-solr-${c} -- curl "${REMOTE_INDEX_FILE_PATH}" --output /opt/gatling/user-files/data/external/${REMOTE_INDEX_FILE_PATH}
  done

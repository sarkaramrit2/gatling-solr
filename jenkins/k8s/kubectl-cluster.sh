...
if [ "$GCP" = true ] ; then
  ...
  docker run -it -d --rm  -e AWS_SECRET_ACCESS_KEY=$(cut -d ' ' -f 1 ./GCP_KEY_FILE) -e AWS_ACCESS_KEY_ID=$(cut -d '  ' -f 2 ./GCP_KEY_FILE) --name kubectl-support sarkaramrit2/kubectl-support:latest
  rm -rf ./GCP_KEY_FILE
fi

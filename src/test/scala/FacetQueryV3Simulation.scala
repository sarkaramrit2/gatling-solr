...
class FacetQueryV3Simulation extends Simulation {
  ...
  object Query {
    ...
    val search = repeat(Config.numQueriesPerUser) {
      feed(feeder).exec(solr("facetQueryRequest")
        // query to do
        .queryV3[String](Config.basequery))
    }
  }

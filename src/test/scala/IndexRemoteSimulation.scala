...
class IndexRemoteSimulation extends Simulation {
  ...
  val solrIndexV2Feeder = new Feeder[util.ArrayList[SolrInputDocument]] {
    ...
    private val indexFile = new URL(Config.indexFilePath)
    private val reader = new BufferedReader(new InputStreamReader(indexFile.openStream()))
